...
impl DocumentMutator<'_> {
    ...
    /// Convert declarative shadow DOM (`<template shadowrootmode="open|closed">`)
    /// into real shadow roots: each such template's parent becomes a shadow host
    /// whose shadow tree is the template's content, and the template element is
    /// removed.
    ///
    /// This should be called after parsing but before the mutator is flushed, so
    /// that any `<style>` elements moved into the shadow tree are routed to the
    /// shadow root's scoped stylesheet set rather than the global stylist.
    #[cfg(feature = "shadow-dom")]
    pub fn process_declarative_shadow_roots(&mut self) {
        use crate::node::ShadowRootMode;

        loop {
            let found = self.doc.nodes.iter().find_map(|(id, node)| {
                let el = node.element_data()?;
                if el.name.local != local_name!("template") {
                    return None;
                }
                let mode = match el.attr(local_name!("shadowrootmode")) {
                    Some("open") => ShadowRootMode::Open,
                    Some("closed") => ShadowRootMode::Closed,
                    _ => return None,
                };
                let parent = node.parent?;
                // The parent must be an element to host a shadow root.
                self.doc.nodes[parent].element_data()?;
                Some((id, parent, mode))
            });

            let Some((template_id, parent_id, mode)) = found else {
                break;
            };

            let shadow_root = self.doc.attach_shadow(parent_id, mode);
            let children = self.doc.nodes[template_id].children.clone();
            self.append_children(shadow_root, &children);
            self.remove_and_drop_node(template_id);
        }
    }
