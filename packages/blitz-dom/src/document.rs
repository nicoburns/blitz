...
impl BaseDocument {
    ...
    pub fn process_style_element(&mut self, target_id: usize) {
        // A `<style>` inside a shadow tree is scoped to that shadow root rather
        // than the global document: invalidate the shadow root's cascade data
        // (rebuilt during resolve) and skip adding it to the global stylist.
        #[cfg(feature = "shadow-dom")]
        if self.nodes[target_id].containing_shadow_root().is_some() {
            self.invalidate_containing_shadow_styles(target_id);
            return;
        }
